...
(function(root){
  const durations={stage:[160,740,200],fusion:[100,240,160],popped:[110,300,190],combo:[130,440,230],notice:[110,300,190]};
  function create({show,hide,blocked=()=>false,now=()=>Date.now(),later=setTimeout,cancel=clearTimeout}){
    ...
    const priority=e=>e.kind==='stage'?3:e.kind==='fusion'?2:1;
  ...
  const images=new Map(),labels={'lets-roll':'LET’S ROLL!',fusion:'FUSION!',popped:'POPPED!',combo:'COMBO'};
  ...
  function layout(){
    ...
    const b=board.getBoundingClientRect(),scale=b.width>=480?Math.min(1.2,b.width/400):1;
    // Only the announcement scales. Reserve space for the 1.07 overshoot and stroke.
    const limit=Math.max(1,Math.min(b.width*.86,b.width-16)/1.07),width=Math.min(260,limit/scale);
  ...
  }).concat([document.fonts.load('400 23px Jua').catch(()=>{})])).then(()=>{loaded=true;const e=waiting;waiting=null;if(e&&Date.now()-e.at<800)controller.emit(e.event);});
