...
(function(root){
  ...
  function plan(width,height,square,gold){
    const id=(square?'node-':'button-')+(gold?'gold':'ivory');
    const [iw,ih,x,y,w,h]=windows[id],dh=height+7;
    if(square){const k=Math.min(width/w,dh/h);return {id,iw,ih,parts:[[x,y,w,h,(width-w*k)/2,(dh-h*k)/2,w*k,h*k]]};}
    const k=dh/h,cap=350*k,mid=width-2*cap,sw=w-700,e=.6;
    if(mid<16)throw Error('Horizontal surface needs wider click box');
    return {id,iw,ih,parts:[[x,y,350+e/k,h,0,0,cap+e,dh],[x+350,y,sw+e*sw/mid,h,cap,0,mid+e,dh],[x+w-350,y,350,h,width-cap,0,cap,dh]]};
  }
  function svg(width,height,square,gold){
    const p=plan(width,height,square,gold);
    ...
    return '<svg xmlns="http://www.w3.org/2000/svg" class="hmSurface" aria-hidden="true" focusable="false" width="'+width+'" height="'+(height+7)+'" data-asset="'+p.id+'">'+p.parts.map(([sx,sy,sw,sh,x,y,w,h],i)=>{
      const slot=square?w:(p.parts[i+1]?.[4]??width)-x;
      return '<svg x="'+x+'" y="'+y+'" width="'+slot+'" height="'+h+'" overflow="hidden"><svg width="'+w+'" height="'+h+'" viewBox="'+[sx,sy,sw,sh].join(' ')+'" preserveAspectRatio="none" overflow="hidden"><image href="assets/home-map/'+p.id+'.png" width="'+p.iw+'" height="'+p.ih+'"/></svg></svg>';
    }).join('')+'</svg>';
  }
  const images=new Map(),rasters=new Map();
  function raster(width,height,square,gold,pixelRatio=root.devicePixelRatio||1){
    if(!root.Image)return null;
    const dpr=Math.min(3,pixelRatio),p=plan(width,height,square,gold),key=[width,height,p.id,dpr].join(':');
    if(rasters.has(key))return rasters.get(key);
    if(!images.has(p.id))images.set(p.id,new Promise((resolve,reject)=>{const img=new root.Image();img.onload=()=>resolve(img);img.onerror=reject;img.src='assets/home-map/'+p.id+'.png';}));
    const result=images.get(p.id).then(img=>{
