...
export function createCubeVisuals(bridge){
  ...
  const iceRenderer=createIceRenderer(scene,camera,geometry,d=>wake(d));
  const vineRenderer=createVineRenderer(scene,camera,board,d=>wake(d));
  ...
  function diagnostic(){output.textContent=JSON.stringify({...stats,sealed:[...records.values()].filter(r=>r.entity.vined).map(r=>({id:r.entity.logicId,mesh:r.mesh.visible,shadow:r.shadow.visible})),active,pending:scheduled?1:0,cubeCount:records.size,glyphMode:approved?'screen-fixed':'during-roll',frameMode:scaffold?'3d-prototype':`delivered-${layout.assetVersion||layout.frameVersion||'v1'}`,lighting:look,triangles:geometry.attributes.position.count/3,canvasSize:side,camera:{angle:Math.asin(layout.sin)*180/Math.PI,fov:layout.fov,position:camera.position.toArray(),principalY:layout.cy}});}
  ...
  function sync(record,now){
    ...
    record.entryPlane.constant=entity.el.classList.contains('falling')?(entity.entryRow===undefined?layout.side/2:-layout.world(0,entity.entryRow*72-32).z):100;
  ...
  function render(now=performance.now()){
    ...
    // The initial placement and responsive fit can finish after the help opens.
    // Track the displayed transform, not an earlier layout snapshot.
    window.GimmickPlay?.positionHelp();
  }
  ...
  function resize(){
    ...
    if(side!==next){side=next;renderer.setSize(side,side,false);}
    // Responsive zoom changes computed transforms over the existing CSS easing.
    // Keep sampling until those transforms settle, not just the first resize frame.
    wake(500);
  }
  ...
  renderer.domElement.addEventListener('webglcontextlost',e=>{e.preventDefault();lost=true;stats.ready=false;window.CubePopVisual=null;vineRenderer.stop();board.classList.remove('board3d');for(const entity of records.keys())entity.el.classList.remove('renderedCube');host.hidden=true;observer.disconnect();diagnostic();});
  ...
  api.outline=entity=>records.has(entity)?iceRenderer.outline(records.get(entity)):null;
  diagnostic();window.GimmickPlay?.positionHelp();return api;
}
